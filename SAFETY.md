...
Internal paper controls use separate mode-`0600`, full-sync SQLite stores that start paused or contain only broker evidence. Pause requires no approval and blocks pre-trade processing before external reads. Every future submission must persist its canonical payload and deterministic client ID, claim one durable attempt, and use lookup-only recovery after uncertainty; no recovery path can submit. Unresolved state blocks every new order. Resume and cancel use distinct expiring, one-use HMAC approvals; cancel affects only deterministic bot-owned paper orders, verifies terminal and residual state, reconciles, and remains paused. The tested crash and failure matrix is published in [`docs/FAILURE_INJECTION.md`](docs/FAILURE_INJECTION.md). Position flattening and every public paper command remain unavailable.
