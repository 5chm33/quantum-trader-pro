...
| BT-08 | Process crash after submit | Unknown external order ownership | Durable submission journal; deterministic client ID; lookup-only recovery; global unresolved-order halt; literal subprocess exit and fresh-process reconciliation | Service restart, boundary-wide `SIGKILL`, power loss, and filesystem loss remain external evidence |
...
| BT-19 | Disk full/corruption | Intent not persisted or evidence lost | Pre-submit durable write; secure regular-file path; SQLite header/integrity checks; simulated `SQLITE_FULL` rollback; fail closed | Physical-volume exhaustion, media failure, and catastrophic disk loss remain external evidence |
...
The deterministic release-candidate evidence now includes transition-table coverage, duplicate-event tests, every local submission crash boundary, literal subprocess termination followed by repeated fresh-process recovery, close/reopen recovery, partial fills, a fill-during-cancel race, non-success HTTP classification, operator pause races, in-progress kill-action restart, stale-data and calendar tests, permission/symlink/secret-redaction tests, SQLite corruption rejection, simulated `SQLITE_FULL`, injected transaction rollback, locked dependency resolution, vulnerability audit, SBOM generation, package checks, and cross-platform launchers. It still requires service-manager and host-loss evidence, authenticated multi-session paper acceptance tied to an exact commit/configuration/account fingerprint, and a separate flattening campaign. Live execution remains unavailable and is not implied by passing any paper gate.
